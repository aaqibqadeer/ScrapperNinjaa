...
| `AppShell`          | `components/shared/AppShell.tsx`          | Signed-in app chrome: collapsible left sidebar + main content area (replaces top header). Server component; derives nav links from flags + role.                                                                                                  | `session`, `children`                                                                                  | All signed-in app pages                                          |
| `AppSidebar`        | `components/shared/AppSidebar.tsx`        | Client collapsible left nav (icons + labels, localStorage collapse state). Footer: email, sign-out, theme toggle, collapse.                                                                                                                         | `links`, `homeHref`, `workspaces`, `activeOrgId`, `userEmail`                                         | `AppShell`                                                       |
| `AppHeader`         | `components/shared/AppHeader.tsx`         | **Deprecated** — re-exports `AppShell`.                                                                                                                                                                                                           | `session`, `children`                                                                                  | _(legacy alias)_                                                 |
| `AppNav`            | `components/shared/AppNav.tsx`            | Client nav link row with active-route highlighting (`usePathname`). `/leads` matches exactly (sub-routes have their own links). Links computed server-side and passed in.                                                                          | `links: {href,label}[]`, `className?`                                        | _(mobile fallback if needed)_                                    |
...
| `ColumnPicker`      | `components/shared/ColumnPicker.tsx`      | Show/hide columns + reorder visible ones via native HTML5 drag-and-drop (no dnd lib); hidden columns listed separately; select all / select none shortcuts.                                                                                      | `columns: {key,label}[]`, `visibleKeys`, `onChange(visibleKeysOrdered)`                                | _(ScrapperNinja Lead Directory)_ |
...
| `SavedViewsMenu`    | `components/shared/SavedViewsMenu.tsx`    | Saved-views dropdown: load, save-current-as (inline input), update active view, rename (pencil + inline edit), set default (star), delete (inline confirm).                                                                                     | `views: {id,name,isDefault}[]`, `activeViewId`, `onLoad`, `onSave`, `onUpdate?`, `onRename?`, `onSetDefault`, `onDelete` | _(ScrapperNinja Lead Directory)_ |
...
| `TableLoadingOverlay` | `components/shared/TableLoadingOverlay.tsx` | Fade-in spinner overlay for in-place table refetches (keeps rows visible underneath).                                                                                                                          | `show`                                                                                                   | `LeadsTable`                                                     |
| `TableLoadingSkeleton` | `components/shared/TableLoadingSkeleton.tsx` | Pulsing placeholder rows for first table load.                                                                                                                                                                  | `rows?`, `className?`                                                                                    | `LeadsTable`, lead sub-pages                                     |
